...
Page({
  ...
  async getCommentDetail(shedule_id, student_id){
    var token = wx.getStorageSync('token');
    const data = await utils.get('/api/shedule_comment/get_list',{
      shedule_id: shedule_id,
      student_id: student_id
    },token);
    var detail=data.data.data[0];
    console.log(detail);
    this.setData({
      detail:detail,
      picboxs: detail.pic,
      commentId: detail.id,
      comentVal: detail.content
    })
  },
  close: function (e) {
  ...
  send: function () {
    ...
    if (content == '' && pics.length == 0) {
      ...
      wx.request({
        url: baseUrl + '/api/shedule_comment/edit',
        method: 'POST',
        header: {
          token: token
        },
  ...
  onLoad: function (options) {
    ...
    this.getCommentDetail(shedule_id, student_id);
  },
